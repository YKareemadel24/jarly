...
import { JarSkeleton } from "@/components/empty-state";
...
import { depositPresets, nextBestJarId } from "@/lib/savings-core";
...
function GoalCard({ jar, styles, colors, accents, format }: { jar: Jar; styles: ReturnType<typeof makeStyles>; colors: ThemeColorPalette; accents: Record<Accent, string>; format: (minor: number) => string }) {
  ...
  return <Pressable
    ...
    onPress={() => { feedback.tap(); router.push(`/jar/${jar.id}` as never); }}
  ...
  >
    ...
    <View style={styles.goalCopy}>
      ...
      <View style={styles.track}><View style={[styles.trackFill, { width: `${progress}%`, backgroundColor: accent }]} /><View style={[styles.trackTip, { left: `${progress}%`, backgroundColor: accent }]} /></View>
...
export default function HomeScreen() {
  ...
  const usual = nextJar && nextPace ? depositPresets(nextJar, nextPace).find((p) => p.kind === "usual")?.amount : undefined;
...
const makeStyles = (c: ThemeColorPalette) => StyleSheet.create({
  ...
  goalCard: { minHeight: 114, backgroundColor: c.surface, borderRadius: 22, borderColor: c.border, borderWidth: 1, padding: 12, flexDirection: "row", alignItems: "center", gap: 10 }, goalVisual: { width: 77, alignItems: "center", justifyContent: "center" }, goalCopy: { flex: 1, alignSelf: "stretch", justifyContent: "center" }, goalHeading: { flexDirection: "row", alignItems: "center", gap: 7 }, goalName: { color: c.foreground, flex: 1, fontSize: 15, fontWeight: "800" }, pacePill: { borderRadius: 999, paddingHorizontal: 7, paddingVertical: 3 }, paceText: { fontSize: 9, fontWeight: "800" }, goalAmount: { color: c.foreground, fontSize: 14, marginTop: 5, fontWeight: "700", fontVariant: ["tabular-nums"] }, goalTarget: { color: c.muted, fontWeight: "500" }, track: { height: 8, borderRadius: 999, backgroundColor: c.border, marginTop: 10 }, trackFill: { height: "100%", borderRadius: 999 }, trackTip: { position: "absolute", top: -2, width: 12, height: 12, borderRadius: 999, marginLeft: -6 }, goalMeta: { color: c.muted, fontSize: 11, marginTop: 8 },
  ...
  quickAddChip: { borderRadius: 11, paddingHorizontal: 11, minHeight: 44, justifyContent: "center" }, quickAddChipText: { fontSize: 12, fontWeight: "800" }, pressed: { opacity: .86, transform: [{ scale: .98 }] },
});
